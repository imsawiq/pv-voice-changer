...
public final class VoiceChangerAddon {
    ...
    private void ensureHotkeyRegistered() {
        ...
        hotkey.addPressListener(action -> {
            if (action == Hotkey.Action.DOWN && MinecraftClient.getInstance().currentScreen == null) {
                toggleEnabled();
            }
        });
    }
    ...
    private static VoiceChangerProfile sanitizeProfile(VoiceChangerProfile profile) {
        return new VoiceChangerProfile(
                ...
                sanitize(profile.bitDepth(), VoiceChangerProfile.BIT_DEPTH_CLEAN, VoiceChangerProfile.BIT_DEPTH_MIN, VoiceChangerProfile.BIT_DEPTH_CLEAN)
        );
    }
